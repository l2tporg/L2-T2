...
# Commands:
#  [省略可能], <引数>
#  [l2-t2] add <URL:string> <STATUS:int> - 検査するサイトを登録
#  [l2-t2] list - 登録されたサイトをインデックス付きで表示
#  [l2-t2] update <INDEX:int> <NEW_STATUS:int> - 登録されたサイトのインデックスと新しいステータスを指定して更新
#  [l2-t2] remove <INDEX:int> - 登録されたサイトをインデックスを指定して削除
#
# Author:
#   @sak39
#
...
#      robot.send {room: "bot"}, "\"#{obj.url}\" status: #{obj.status}" #@@
      robot.send "\"#{obj.url}\" status: #{obj.status}" #@@
